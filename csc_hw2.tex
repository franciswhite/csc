...
\section*{2}
It is possible to find an SCF that satisfies the three desiderate for any $n=x*6$, where $x\in \mathbb{N}$. For all $n$ that are multiples of $6$ no such SCF can be found.\\
One voting rule that has all the desired properties:\\
The candidate(s) who win(s) the most pair-wise contests across the profile is/are elected.\\
For $n\neq x\in \mathbb{N}*6$, this rule respects all desiderata: Anonymity, for all voters are considered equally important; neutrality, for the candidates are treated equally; lastly, it can never end in a three-way-tie. To see this, consider what would be necessary to obtain a three-way-tie for any profile $\mathbf{R}$: all possible orderings (permutations) of the three candidates must occur equally many times in $\mathbf{R}$. But there are $3!=6$ many possible such permutations, so for each of them to occur the same number of times there would need to be $n=x\in \mathbb{N}*6$ voters. But we excluded precisely these values for $n$.\\

\noindent It is impossible to find any SCF $\mathbf{F}$
