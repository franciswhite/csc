...
\section*{2}
It is possible to find an SCF that satisfies the three desiderate for any $n=x*6$, where $x\in \mathbb{N}$. For all $n$ that are multiples of six no such SCF can be found.\\ %This should be n=x*3

\noindent One voting rule that has all the desired properties:\\
\textbf{The candidate(s) who win(s) the most pair-wise contests across the profile is/are elected.}\\ %This is essentially Borda!
For $n\neq x\in \mathbb{N}*6$, this rule respects all desiderata: \textit{Anonymity}, for all voters are considered equally important; \textit{neutrality}, for the candidates are treated equally; lastly, it can never end in a \textit{three-way tie}. To see this, consider what would be necessary to obtain a \textit{three-way tie} for any profile $\mathbf{R}$: all possible orderings (permutations) of the three candidates in the candidates set $X$ must occur equally many times in $\mathbf{R}$. %Is this so? ->NO! (A>B>C,C>B>A). But there are $3!=6$ many possible such permutations, so for each of them to occur the same number of times there would need to be $n=x\in \mathbb{N}*6$ voters. But we excluded precisely these values for $n$.\\

\noindent It is impossible to find any SCF $\mathbf{F}$ that satisfies the three desiderata for $n=x*6$ voters, where $x\in \mathbb{N}$.\\
For any $\mathbf{F}$ that satisfies \textit{anonymity} and \textit{neutrality} we can construct a profile $\mathbf{R}^*$ with $n=x\in \mathbb{N}*6$ voters that ends in a \textit{three-way tie}. $\mathbf{R}^*$ consists of $x\in \mathbb{N}$ copies of all $3!=6$
ways to order $X$. That is, all possible ordering so $X$ are present in equal number. Since $\mathbf{F}$ is \textit{anonymous} the order of the ballots in $\mathbf{R}^*$ cannot matter; since it is \textit{neutral} the specific candidate under inspection is irrelevant for the output of $\mathbf{F}$. Therefore, such an $\mathbf{R}^*$ should output a set of three winners. We have a \textit{three-way tie}.
...
\textbf{Example:} The \textit{antiplurality} rule (lexicographic tie-breaking) would be \textit{surjective} and \textit{resolute} but fail to be \textit{paretian}.
