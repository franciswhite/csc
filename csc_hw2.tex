...
This measure of distance between two profiles - $\boldsymbol{R}$ and $\boldsymbol{R'}$ -  only takes into account swaps from one end of an individual ranking to the other. The \textit{Cindarella-Swap Distance} between $\boldsymbol{R}$ into $\boldsymbol{R'}$ is the minimal number of such extremely-opposed swaps needed to turn one into the other.
...
To see this, observe that there is no SCF for $n=3$ that respects \textit{neutrality} and \textit{anonymity}. consider the following profile:
...
b \succ c \succ a \\
\end{align*}
So we find ourselves with the original profile where $a$ won. We have encountered a contradiction.\\ 
