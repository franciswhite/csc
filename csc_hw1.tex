...
\subsection*{c}
Let $X=\{1,...,m\}$ be the set of candidates, $N=\{1,...,n\}$ the set of voters, $\mathcal{L}(X)$ the set of strict orderings over $X$ and  $\mathcal{L}(X)^n$ the set of possible voter profiles \footnote{Wikipedia claims that $n\geq 25$ and $m\geq 4$} is needed to.\\
If a voting rule $F$ always picks an $x\in X$ that is preferred by a majority of voters against each $y\in X-\{x\}$, then there are profiles in $\mathcal{L}(X)^n$ where some $i \in N$ has an incentive to abstain from voting.
\subsection*{d}
